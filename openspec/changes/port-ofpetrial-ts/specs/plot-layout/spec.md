...
La bibliothèque SHALL exposer une fonction `makeExpPlots` équivalente à `ofpetrial::make_exp_plots` : à partir d'un ou plusieurs `PlotInfo`, d'un contour de champ (GeoJSON Polygon/MultiPolygon, WGS84) et d'une ab-line **requise** (GeoJSON LineString, WGS84 ; `ablineType: "free" | "lock" | "non"` — `"non"` est la valeur R réelle (vérifié par exécution : R produit alors des `ab_lines` NULL ; le port TS calcule l'ab-line comme en mode "free" car `InputLayout.abLine` est non-nullable — déviation documentée). Toute autre chaîne (dont `"none"`, documentée mais invalide en R 0.1.3, où elle fait planter la construction) lève une `ValidationError`), elle produit un `ExpData` contenant, par intrant : les parcelles expérimentales (polygones avec `plot_id`, `strip_id`), les tournières (headlands), l'ab-line utilisée et les lignes de guidage (si aucune ligne de guidage n'est générée, `guidanceLines` est une `FeatureCollection` vide — jamais `null`/`undefined`). Le calcul MUST se faire en coordonnées UTM métriques (zone déterminée depuis le centroïde du champ, comme en R), les résultats étant restitués en WGS84.
